...
- **Phase Separation**:
  ...
  - **Phase 2 (Active Implementation Plan)**: Dedicated web portal intake, deterministic stage-gate orchestration with Vercel Workflow SDK, interactive React Flow visualization, separate chat routing, and Blob-backed case state. Operates side by side with Phase 1 via an explicit opt-in trigger. Refer to [PHASE_TWO_PLAN.md](./PHASE_TWO_PLAN.md) for the active Phase 2 execution checklist.
